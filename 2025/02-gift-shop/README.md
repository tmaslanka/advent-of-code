...
We're given inclusive ID ranges like `11-22, 95-115, 998-1012`, and every range
we have to find IDs/numbers that are essentially doubles of digit subsequences
in that range, each subsequence starts from first digit.

The brute force solution is to for every range `R` iterate over numbers
`num` in `R` and check whether `num` length is even, and `num` is palindrome.
...
E.g. `1212` where repeated part is `12`, or `11` where repeated part is `1`,
or `123456123456` where repeated part is `123456, or
123123123` where repeated part is `123` 3 times.

Observations:
 * number can be of a * b size where a is repetitions cout and b is subsequence
   length. Since a >= 2 and b >= 1 the num length is not a prime number. (Not useful).
